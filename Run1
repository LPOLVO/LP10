-- LP10 HUB | SELF-RELOADING WRAPPER
-- يخزّن السكربت كاملاً في getgenv + TeleportSetting + ملف
-- حتى يُعاد تشغيله تلقائياً بعد Replay/Teleport مع كل الإعدادات.
local LP10SelfEnv = getgenv and getgenv() or _G
local LP10AutomationSource = [===[
...
local Config = {
    ...
    MinSize = Vector2.new(180, 130),
    MaxSize = Vector2.new(480, 400)
}

local LP10BootstrapEnv = getgenv and getgenv() or _G
if LP10BootstrapEnv.LP10AutomationGui and LP10BootstrapEnv.LP10AutomationGui.Parent then
    pcall(function() LP10BootstrapEnv.LP10AutomationGui:Destroy() end)
end

local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "LP10_Final_Edition"
ScreenGui.Parent = CoreGui
ScreenGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
ScreenGui.ResetOnSpawn = false
LP10BootstrapEnv.LP10AutomationGui = ScreenGui
...
local LP_Label = Instance.new("TextLabel", ToggleBtn)
LP_Label.Size = UDim2.new(1, 0, 1, 0)
LP_Label.BackgroundTransparency = 1
LP_Label.Text = "LP"
LP_Label.Font = Enum.Font.FredokaOne
LP_Label.TextColor3 = Color3.fromRGB(255, 255, 255)
LP_Label.TextScaled = true
local UIGradient = Instance.new("UIGradient", LP_Label)
UIGradient.Color = ColorSequence.new({ColorSequenceKeypoint.new(0, Config.AccentColor), ColorSequenceKeypoint.new(1, Color3.fromRGB(0, 100, 200))})
Instance.new("UIPadding", LP_Label).PaddingBottom = UDim.new(0, 10)
    ...
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        ...
        connection = UserInputService.InputChanged:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
                local delta = input.Position - dragStart
                local newX = math.clamp(startSize.X.Offset + delta.X, Config.MinSize.X, Config.MaxSize.X)
                local newY = math.clamp(startSize.Y.Offset + delta.Y, Config.MinSize.Y, Config.MaxSize.Y)
...
ToggleBtn.MouseButton1Click:Connect(function()
    ...
    if isOpen then
        Main.Visible = true
        Main:TweenSize(currentSize, "Out", "Back", 0.3, true)
    else
...
print("LP10 HUB: Royal Blue Edition Loading...")
...
-- ============================================================================
-- SECTION C: AUTOFARM — FULLY PORTED FROM RUNAWAYS.lua
-- ============================================================================
library = library or {}
...
function AutoFarm:GetTeleportLoader()
    -- bootstrap صغير يُعيد تحميل LP10 نفسه بعد Replay/Teleport
    -- يحفظ كل الإعدادات (Auto Farm, SafeGateWait, etc.)
    return [==[
local src = nil
...
-- ============================================================================
-- SECTION D: LP10 UI PANEL (AutoFarm Wired In)
-- ============================================================================
LP10UI = {
    toggles = {
        autoFarm = false,
...
AutomationPanel.Position = UDim2.new(0, 8, 0, 40)
AutomationPanel.Size = UDim2.new(1, -16, 1, -56)
AutomationPanel.BackgroundTransparency = 1
AutomationPanel.BorderSizePixel = 0
AutomationPanel.ScrollBarThickness = 4
AutomationPanel.ScrollBarImageColor3 = Config.AccentColor
...
local function makeToggle(text, default, callback)
    ...
    local function render()
        ...
        btn.BackgroundColor3 = state and Color3.fromRGB(20, 92, 145) or Color3.fromRGB(25, 25, 25)
    end
    render()

    local setter
    setter = function(newState, fire)
        state = newState == true
        render()
...
makeLabel("AUTOMATION", 22, Config.AccentColor)
...
makeLabel("Delay Between Runs", 16, Color3.fromRGB(170, 170, 170))
...
makeLabel("Gate Timeout", 16, Color3.fromRGB(170, 170, 170))
...
makeLabel("Retry Delay", 16, Color3.fromRGB(170, 170, 170))
...
Footer.TextColor3 = Config.AccentColor
Footer.TextSize = 10
Footer.TextXAlignment = Enum.TextXAlignment.Right

-- ============================================================================
-- SECTION E: LIVE SYNC LOOP
-- ============================================================================
...
print("LP10 HUB: RUNAWAYS AutoFarm (Full Port) loaded | by LPOLVO")
]===]

LP10SelfEnv.LP10RunawaysAutomationSource = LP10AutomationSource

-- نحفظ المصدر في TeleportSetting (يستمر بين السيرفرات)
pcall(function()
    game:GetService("TeleportService"):SetTeleportSetting("LP10_AUTOMATION_SOURCE", LP10AutomationSource)
end)

-- نحفظ المصدر في ملف كنسخة احتياطية
if type(writefile) == "function" then
    pcall(function()
        if type(isfolder) ~= "function" or not isfolder("MyScriptHub") then
            if type(makefolder) == "function" then makefolder("MyScriptHub") end
        end
        writefile("MyScriptHub/LP10_Automation.lua", LP10AutomationSource)
    end)
end

-- الآن نشغّل السكربت
local LP10Run = loadstring(LP10AutomationSource)
if type(LP10Run) == "function" then
    LP10Run()
end
